...
def calculate_conditional_mutual_information_domain(domain, given_that, X_idx, Y_idx, X_range_list, Y_range_list):
    # creating the domain filter map
    filter_mask = np.ones(domain.shape[0], dtype=bool)
    for key, value in given_that.items():
        filter_mask = np.logical_and(filter_mask, domain[:, key] == value)

    filtered_domain = domain[filter_mask]
    if filtered_domain.shape[0] == 0:
        return 0.0

    mutual_information = calculate_mutual_information_domain(filtered_domain, X_idx, Y_idx, X_range_list, Y_range_list)
    return mutual_information * (filtered_domain.shape[0] / domain.shape[0]), (
                filtered_domain.shape[0] / domain.shape[0])
